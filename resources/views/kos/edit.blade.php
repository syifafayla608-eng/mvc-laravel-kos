...
<head>
    <meta charset="UTF-8">
    <title>Edit Kos - Fayla Residence</title>
...
<div class="container mt-4 mb-5" style="max-width:600px">
    <div class="card shadow">
        <div class="card-header">
            <h5><i class="bi bi-pencil me-2"></i>Edit Data Kos</h5>
        </div>
        <div class="card-body p-4">
            ...
            <form action="{{ route('kos.update', $kos->id) }}" method="POST">
                ...
                <div class="mb-4">
                    ...
                    <input type="text" name="nama_kos" class="form-control @error('nama_kos') is-invalid @enderror" value="{{ old('nama_kos', $kos->nama_kos) }}">
                    @error('nama_kos') <div class="invalid-feedback">{{ $message }}</div> @enderror
                </div>
                <div class="mb-4">
                    <label class="form-label">Alamat</label>
                    <input type="text" name="alamat" class="form-control @error('alamat') is-invalid @enderror" value="{{ old('alamat', $kos->alamat) }}">
                    @error('alamat') <div class="invalid-feedback">{{ $message }}</div> @enderror
                </div>
                <div class="mb-4">
                    <label class="form-label">Harga / Bulan (Rp)</label>
                    <input type="number" name="harga" class="form-control @error('harga') is-invalid @enderror" value="{{ old('harga', $kos->harga) }}">
                    @error('harga') <div class="invalid-feedback">{{ $message }}</div> @enderror
                </div>
                <div class="mb-4">
                    <label class="form-label">Jumlah Kamar</label>
                    <input type="number" name="jumlah_kamar" class="form-control @error('jumlah_kamar') is-invalid @enderror" value="{{ old('jumlah_kamar', $kos->jumlah_kamar) }}">
