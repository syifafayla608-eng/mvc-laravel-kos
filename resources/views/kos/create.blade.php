...
<head>
    <meta charset="UTF-8">
    <title>Tambah Kos - Fayla Residence</title>
    ...
    <style>
        * { font-family: 'Inter', sans-serif; }
        ...
        .form-control { border: 1.5px solid #dde3ef; border-radius: 10px; padding: 11px 16px; color: #1a2f5e; background: #f8f9fd; }
        .form-control:focus { border-color: #c9a84c; box-shadow: 0 0 0 3px #c9a84c22; background: #fff; }
...
<div class="container mt-4 mb-5" style="max-width:600px">
    <div class="card shadow">
        <div class="card-header">
            <h5><i class="bi bi-plus-circle me-2"></i>Tambah Data Kos</h5>
        </div>
        <div class="card-body p-4">
            ...
            <form action="{{ route('kos.store') }}" method="POST">
                ...
                <div class="mb-4">
                    ...
                    <input type="text" name="nama_kos" class="form-control @error('nama_kos') is-invalid @enderror" placeholder="cth: Kos Melati" value="{{ old('nama_kos') }}">
                    @error('nama_kos') <div class="invalid-feedback">{{ $message }}</div> @enderror
                </div>
                <div class="mb-4">
                    <label class="form-label">Alamat</label>
                    <input type="text" name="alamat" class="form-control @error('alamat') is-invalid @enderror" placeholder="cth: Jl. Banda Aceh No.1" value="{{ old('alamat') }}">
                    @error('alamat') <div class="invalid-feedback">{{ $message }}</div> @enderror
                </div>
                <div class="mb-4">
                    <label class="form-label">Harga / Bulan (Rp)</label>
                    <input type="number" name="harga" class="form-control @error('harga') is-invalid @enderror" placeholder="cth: 700000" value="{{ old('harga') }}">
                    @error('harga') <div class="invalid-feedback">{{ $message }}</div> @enderror
                </div>
                <div class="mb-4">
                    <label class="form-label">Jumlah Kamar</label>
                    <input type="number" name="jumlah_kamar" class="form-control @error('jumlah_kamar') is-invalid @enderror" placeholder="cth: 10" value="{{ old('jumlah_kamar') }}">
                    @error('jumlah_kamar') <div class="invalid-feedback">{{ $message }}</div> @enderror
                </div>
                <div class="d-flex gap-2 mt-3">
                    <a href="{{ route('kos.index') }}" class="btn btn-outline-navy">Batal</a>
                    <button type="submit" class="btn btn-gold"><i class="bi bi-save me-1"></i>Simpan</button>
                </div>
            </form>
        </div>
    </div>
